...
def extract_gcode_paths():
    ...
    # IMPROVED: More comprehensive G-code pattern matching
    # This will match G0, G1, G01, G001, etc. and various formats
    ...
    for idx, row in df.sort_values("timestamp").iterrows():
        ...
        if idx < 5:  # Only print first 5 for debugging
            print(f"[DEBUG] Row {idx}: Type='{packet_type}', Payload='{payload[:50]}...'")
        ...
        if re.search(r'[Gg]\s*[01]', payload, re.IGNORECASE):
            ...
            print(f"[DEBUG] Added {packet_type} position: ({x_val}, {y_val}, {z_val})")
