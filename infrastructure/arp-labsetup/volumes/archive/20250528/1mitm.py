...
def check_control_file():
    global poisoning_enabled, sniffer_enabled, sniffer_thread, arp_poison_thread, stop_poison_flag
    ...
    while True:
        try:
            ...
            try:
                ...
                # Keep track of the connection's TCP flags by sniffing for related packets
                connection_flags = None
                if sniffer_enabled:
                    # Try to get the most recent packet for this connection from sniffed packets
                    # This is a placeholder - in a real implementation, we would need a cache of recent packets
                    connection_flags = "PA"  # Default to PSH+ACK as a common flag combination
                ...
                try:
                    # Try to parse raw packet for TCP headers
                    # Since we're getting data from a socket, not raw packet capture,
                    # we'll use default values but try to extract what we can using Scapy
                    headers = {
                        "src_ip": "10.9.0.5",
                        "dst_ip": "10.9.0.6",
                        "src_port": client_port,
                        "dst_port": 9090,
                        "flags": connection_flags if connection_flags else "PA",  # Default to PSH+ACK
                        "seq": 0,
                        "ack": 0
                    }
                    ...
                    # Log original packet details with TCP flag information
                    log_packet_details(
                        packet_data=payload, 
                        is_original=True,
                    ...
                    # Log the modified packet details with the same TCP flag information
                    log_packet_details(
            ...
            time.sleep(1)  # Avoid tight loop if persistent error
